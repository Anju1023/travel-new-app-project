		...
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			>
				...
				<div className="px-8 py-6 border-b border-white/20 flex justify-between items-center bg-white/30">
					...
					<button
						onClick={onClose}
						className="w-8 h-8 flex items-center justify-center rounded-full bg-black/5 text-gray-500 hover:bg-black/10 transition-colors"
					>
						✕
					</button>
				</div>
					...
					<AnimatePresence mode="wait">
						{!result ? (
							...
							<motion.div
								...
								className="space-y-6"
							>
								<div className="bg-white/40 p-6 rounded-4xl border border-white/60 shadow-inner">
										...
										{result.tags && (
											<div className="flex flex-wrap gap-2 mt-4">
												{result.tags.map((tag) => (
													<span
														key={tag}
														className="px-3 py-1 bg-white/80 border border-white/60 rounded-xl text-[10px] font-bold text-gray-500 shadow-sm"
													>
														#{tag}
													</span>
												))}
