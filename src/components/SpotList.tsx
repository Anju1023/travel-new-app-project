...
export default function SpotList({ spots, isLoading, onSpotSelect, onSpotDelete, selectedSpotId }: SpotListProps) {
  if (isLoading) {
    return (
        ...
        <AnimatePresence mode="popLayout">
          {spots.length === 0 ? (
            <motion.div 
              ...
              className="text-center py-20 bg-white/30 rounded-[2rem] border border-dashed border-gray-300"
            >
            ...
            spots.map((spot, index) => (
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={`group p-5 rounded-[1.5rem] cursor-pointer transition-all border relative ${
              ...
              >
                <div className="flex justify-between items-start gap-2 mb-2">
                  <h3 className="font-black text-gray-900 leading-tight text-lg group-hover:text-primary transition-colors pr-8">
                    {spot.name}
                  </h3>
                  <button
                    onClick={(e) => spot.id && handleDelete(e, spot.id)}
                    className="absolute top-5 right-5 p-2 rounded-full bg-red-50 text-red-400 opacity-0 group-hover:opacity-100 hover:bg-red-500 hover:text-white transition-all duration-200"
                  >
                ...
                <p className="text-xs text-gray-500 line-clamp-1 flex items-center gap-1.5 opacity-80">
                  <MapPin className="w-3 h-3 text-primary" /> {spot.address}
                </p>
